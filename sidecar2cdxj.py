...
def create_cdxj_path(sidecar_file, archive_dir):
    """Creates the path and filename for the CDXJ."""
...
def convert_payload_to_dict(record):
    """Takes a record's payload and puts the fields into a dictionary."""
...
def record_data_to_string(record, new_dict):
    """Converts record fields and JSON object into a string."""
...
def create_sidecar_cdxj(sidecar_file, archive_dir):
    ...
    Iterates the metadata records of a WARC metadata sidecar file,
    and writes a line to a CDXJ file containing the SURT-formatted URI,
    ...
    with open(cdxj_path, 'wt') as out, open(sidecar_file, 'rb') as stream:
        for record in ArchiveIterator(stream):
            ...
            record_string = record_data_to_string(record, new_dict)
